    ...
    <SectionWrapper id="about">
      ...
      <div ref={ref} className="grid lg:grid-cols-2 gap-16">
        ...
        >
          ...
          <p className="text-white/60 text-sm leading-loose font-light mb-8">
            Currently deepening expertise in Deep Learning, Transformers, and MLOps — always learning,
            always building.
          </p>
          ...
          <div className="space-y-0 border-t border-white/8">
            {info.map((item) => (
              <div key={item.label} className="flex items-center gap-6 py-3 border-b border-white/8">
                <span className="text-white/30 text-xs tracking-widest uppercase w-20 flex-shrink-0">{item.label}</span>
        ...
        >
          <div className="grid grid-cols-2 gap-px bg-white/8">
            {stats.map((s) => (
              <div key={s.label} className="bg-black p-8 flex flex-col justify-between">
                <span className="text-4xl font-thin text-white">{s.value}</span>
                <span className="text-white/30 text-xs tracking-widest uppercase mt-3">{s.label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="border border-white/8 p-6">
            <p className="text-white/30 text-xs tracking-widest uppercase mb-4">Currently Learning</p>
            <div className="flex flex-wrap gap-2">
              {["Deep Learning", "PyTorch", "Transformers", "CNNs", "RNNs", "MLOps"].map((t) => (
                <span key={t} className="px-3 py-1 text-xs text-white/50 border border-white/10 hover:text-white hover:border-white/30 transition-colors cursor-default">
                  {t}
                </span>
              ))}
            </div>
